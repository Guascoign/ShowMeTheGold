...
def export_to_txt(directory, filename, data, formatted_date):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(f"Alliance - 辛迪加 - {formatted_date}\n")
        f.write("物品ID\t最低价格\t平均价格\t拍卖数量\t物品数量\t数据更新时间\n")
        for item in data:
            #f.write(f"{item['item_name']}\t{conv2Gold(item['min_price'])}\t{conv2Gold(item['ave_price'])}\t{item['auction_num']}\t{item['quantity']}\t{item['scan_time']}\n")
